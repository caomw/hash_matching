...
public:
  ...
  /** \brief Sort 2 pairs by size
    ...
    * \param pair 2
    */
  static bool sortByDistance(const pair<float, string> d1, const pair<float, string> d2)
  {
                                      ...
                                      int b_num_feautres)
  {
    ...
    for (int i=0; i<bucket_cols*bucket_rows; i++)
    {
      ...
      sort(buckets[i].begin(), buckets[i].end(), hash_matching::OpencvUtils::sortDescByDistance);
